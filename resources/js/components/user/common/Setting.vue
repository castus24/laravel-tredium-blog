...
const state = reactive({
    userControls: [
        { title: 'Content filtering', subtitle: 'Set the content filtering level to restrict apps that can be downloaded' },
...
<template>
    <v-card class="mx-auto" max-width="400">
        ...
        <v-list lines="three">
            ...
            <v-list-item
                v-for="item in state.userControls"
                :key="item.value"
        ...
        <v-list
            v-model:selected="state.settingsSelection"
            lines="three"
            select-strategy="leaf"
        >
            <v-list-subheader>General</v-list-subheader>
            <v-list-item
                v-for="item in state.settingsItems"
                :key="item.value"
                :subtitle="item.subtitle"
                :title="item.title"
                :value="item.value"
